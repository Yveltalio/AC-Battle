...
Personnage ChoisirGentil()
{
    ...
    //std::cout.rdbuf(&buf);
    int choix = 0;
    std::vector<Attaque> vect_att = AttributionAttaque("Altair Ibn La-Ahad");
    std::vector<Deffense> vect_def = AttributionDefense("Altair Ibn La-Ahad");
    Personnage LePerso = Personnage(100, 100, "Altair Ibn La-Ahad", true, vect_att, vect_def);
...
Personnage ChoisirPerso(int camp, string Pseudo)
{
    ...
    if (camp == 0)
    {
        //while (choix != 1 || choix != 2)
            std::cout << "Choisissez un cote" << std::endl << std::endl << "1 - Gentil             2 - Malveillant"<<std::endl;
            std::cin >> choix;
